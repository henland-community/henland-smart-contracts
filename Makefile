# Makefile for Henland Smart Contracts
#
# Usage example:
#
...
# The environment variable HENLAND_SC_PARAMS holds flags separated by ':' which
# can be used to control metadata and test features. Leaving this blank should
# cause contracts to run without the henland_sc package. This should not affect
...
ifeq ($(HENLAND_SC_PARAMS),)
  export HENLAND_SC_PARAMS:=tzip16_error_inline:tzip16_error_lint
endif
...
compile_all:
	$(call COMPILE, henlandMarketplace_v1)
...
test_all:
	$(call TEST, henlandMarketplace_v1)
