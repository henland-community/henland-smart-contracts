"""A multisig / mini-DAO contract for the initial henland DAO phase.
...
class DaoMultisig(sp.Contract):
    ...
    def vote_proposal(self, vote):
        ...
        with sp.if_(self.data.votes.get((vote.proposal_id, sp.sender), default_value=False)):
    ...
    def execute_proposal(self, proposal_id):
        ...
        with sp.if_(proposal.value.kind.is_variant("transfer_mutez")):
            with sp.for_("mutez_transfer", proposal.value.mutez_transfers.open_some()) as mutez_transfer:
        ...
        with sp.if_(proposal.value.kind.is_variant("minimum_votes")):
        ...
        with sp.if_(proposal.value.kind.is_variant("expiration_time")):
