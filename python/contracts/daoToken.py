...
class DAOToken(sp.Contract):
    ...
    CONTRACT_METADATA_BASE = {
        "name": "Henland DAO FA2 token contract",
        "description": "DAO token contract used in the Henland DAO",
        "version": "1.0.0",
        "authors": ["Henland Community <https://twitter.com/HenlandCommunity>"],
        "homepage": "https://henland.xyz",
        "source": {
            "tools": ["SmartPy 0.16.0"],
            "location": "https://github.com/henland-community/henland-smart-contracts/blob/main/python/contracts/daoToken.py"
        },
        "license": {
            "name": "MIT",
        ...
        "errors": [ {"error": {"string": "FA2_TOKEN_UNDEFINED"},
                    ...
                    {"error": {"string": "FA2_NOT_OPERATOR"},
                     "expansion": {"string": "The account that executed the entry point is not one of the token operators"},
                     "languages": ["en"]},
                    {"error": {"string": "FA2_INSUFFICIENT_BALANCE"},
                     "expansion": {"string": "The account doesn't have enough token editions"},
                     "languages": ["en"]},
...
# With smartpy-cli and henland_sc we can do some coverage and add further error tests:
if "tzip16_error_inline" in environ.get("HENLAND_SC_PARAMS", "").split(":"):
    from henland_sc.error_collection import ErrorCollection
